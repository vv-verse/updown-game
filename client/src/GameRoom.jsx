/**
 * GameRoom.jsx v5
 * - Voice chat (WebRTC) integrated — join/leave/mute buttons in top bar
 * - Desktop: 3-column layout (scores | game | chat)
 * - Mobile: bottom tab bar (Game / Chat / Scores)
 * - Mic error toast, voice peer count shown
 * - All roomCodeRef.current emits retained
 */
...
function MicIcon({ size = 16, color = '#c8ff00' }) {
  return (
...
function MicOffIcon({ size = 16, color = '#ffaa00' }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none"
      stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        borderRadius:8, padding:'28px 28px 24px', maxWidth:340, width:'100%',
      }}>
        ...
        <p style={{ fontFamily:'sans-serif', fontSize:15, color:'rgba(245,240,232,0.6)', lineHeight:1.6, marginBottom:24 }}>
          You'll exit the game. Your teammates will still be playing.
        </p>
        <div style={{ display:'flex', gap:10 }}>
          <button onClick={onCancel} style={{
            ...
            color:'rgba(245,240,232,0.7)', borderRadius:4,
          }}>Stay</button>
          <button onClick={onConfirm} style={{
            flex:1, padding:'11px', fontFamily:'monospace', fontSize:13,
            letterSpacing:1, textTransform:'uppercase', cursor:'pointer',
            background:'rgba(255,51,102,0.15)', border:'1px solid #ff3366',
            color:'#ff3366', borderRadius:4,
          }}>Leave</button>
        </div>
      </div>
    </div>
...
// ── Scores panel ──────────────────────────────────────────────────────
function ScoresPanel({ room, myId, picker, colorMap, timerLabel }) {
  return (
    <div style={{ padding:'16px', overflow:'auto', height:'100%' }}>
      ...
      <div style={{ padding:'12px', border:'1px solid rgba(245,240,232,0.07)', borderRadius:4, background:'rgba(245,240,232,0.02)' }}>
        ...
        {[
          ['Range', <span key="r" style={{ color:'#c8ff00', fontWeight:'bold' }}>{room.range.min} – {room.range.max}</span>],
          ['Timer', timerLabel],
          ['Room',  room.code],
        ].map(([k,v]) => (
...
function ChatPanel({ chatMessages, chatInput, setChatInput, sendChat, sendEmoji, myId, isDesktop }) {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', height:'100%' }}>
      <div ref={scrollRef} style={{ flex:1, overflow:'auto', padding: isDesktop ? '16px 20px' : '12px 16px', display:'flex', flexDirection:'column', gap:10 }}>
        ...
        {chatMessages.map(msg => (
          <div key={msg.id} style={{ display:'flex', flexDirection:'column', alignItems: msg.playerId === myId ? 'flex-end' : 'flex-start' }}>
            ...
            <span style={{
              ...
              fontSize: msgFont, lineHeight:1.5,
      ...
      <div style={{ display:'flex', flexWrap:'wrap', gap:6, padding: isDesktop ? '8px 16px' : '6px 14px', borderTop:'1px solid rgba(245,240,232,0.08)' }}>
        {EMOJI_LIST.map(e => (
          <button key={e} onClick={() => sendEmoji(e)}
            style={{ fontSize: isDesktop ? 22 : 20, background:'none', border:'none', cursor:'pointer', padding:'2px 4px', lineHeight:1 }}>
            {e}
          </button>
        ))}
      </div>
      ...
      <div style={{ padding: isDesktop ? '10px 14px' : '8px 12px', borderTop:'1px solid rgba(245,240,232,0.08)', display:'flex', gap:8 }}>
        ...
        <button className="btn-primary"
          ...
          onClick={sendChat} disabled={!chatInput.trim()}>↑</button>
      </div>
    </div>
  );
}
...
// ── Game panel ────────────────────────────────────────────────────────
function GamePanel({ room, myId, isPicker, isHost, picker, guess, setGuess, secretInput, setSecretInput, lastHint, inputShake, roundResult, colorMap, lo, hi, submitSecret, submitGuess, handleNextRound, guessInputRef, isDesktop }) {
  return (
      ...
      {room.state === 'roundEnd' && roundResult && (
        <div className="animate-pop">
          {roundResult.winnerId ? (
            ...
            <>
              <h2 style={{ fontFamily:'var(--font-display,monospace)', fontSize: isDesktop ? 48 : 36, color:'#ffaa00', lineHeight:1.05, letterSpacing:2 }}>Time's Up!</h2>
              <p style={{ fontFamily:'monospace', fontSize: isDesktop ? 14 : 12, color:'rgba(245,240,232,0.4)', marginTop:5 }}>Nobody guessed in time.</p>
            </>
          )}
        </div>
      )}
      ...
      {room.state === 'picking' && isPicker && (
        <form onSubmit={submitSecret} style={{ display:'flex', gap:10, maxWidth: isDesktop ? 420 : '100%' }}>
          ...
          <button type="submit" className="btn-primary"
            ...
            disabled={!secretInput}>Lock In</button>
        </form>
      )}
      {room.state === 'picking' && !isPicker && (
      ...
      {room.state === 'guessing' && !isPicker && (
        <div>
          ...
          <form onSubmit={submitGuess}
            ...
            <input ref={guessInputRef} type="number" inputMode="numeric" className="input-field"
              style={{ flex:1, fontSize: isDesktop ? 28 : 24, textAlign:'center', fontFamily:'monospace' }}
      ...
      <div style={{ flex:1 }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:5 }}>
          {(!room.guessHistory || room.guessHistory.length === 0) && (
            <p style={{ fontFamily:'monospace', fontSize: isDesktop ? 13 : 12, color:'rgba(245,240,232,0.2)' }}>No guesses yet.</p>
          )}
          {[...(room.guessHistory || [])].reverse().map((entry, i) => (
            <div key={`${entry.timestamp}-${i}`}
              style={{ display:'flex', alignItems:'center', gap:10, padding: isDesktop ? '10px 14px' : '8px 12px', border:'1px solid rgba(245,240,232,0.06)', background:'rgba(245,240,232,0.03)', borderRadius:4 }}
...
// ── MAIN COMPONENT ────────────────────────────────────────────────────
export default function GameRoom({ room: initialRoom, setRoom, myId, roomCodeRef, voiceEnabled }) {
  ...
  useEffect(() => {
    ...
    const onNumberPicked = ({ room }) => {
      updateRoom(room); setLastHint(null); setGuess(''); setSecretInput(''); setRoundResult(null); setActiveTab('game');
    };
    const onWaitingForPick = ({ room }) => {
      updateRoom(room); setLastHint(null); setGuess(''); setSecretInput(''); setRoundResult(null);
    };
    ...
    const onRoundWon = ({ winnerId, winnerName, secretNumber, room }) => {
      updateRoom(room); setRoundResult({ winnerId, winnerName, secretNumber }); sounds.correct();
    };
    const onTimeUp = ({ room }) => {
      updateRoom(room); setRoundResult({ winnerId: null, winnerName: null, secretNumber: null }); sounds.error();
    };
    const onNextRound = ({ room }) => {
      updateRoom(room); setRoundResult(null); setLastHint(null); setActiveTab('game');
    };
    const onChatMessage = (msg) => {
      setChatMessages(prev => [...prev, msg]);
      setActiveTab(prev => {
        if (prev !== 'chat' && !isDesktop) setUnread(u => u + 1);
        return prev;
      });
    };
  ...
  const panelProps = { room, myId, picker, colorMap, timerLabel };
  const gameProps  = { ...panelProps, isPicker, isHost, guess, setGuess, secretInput, setSecretInput, lastHint, inputShake, roundResult, lo, hi, submitSecret, submitGuess, handleNextRound, guessInputRef };
  const chatProps  = { chatMessages, chatInput, setChatInput, sendChat, sendEmoji, myId };

  // ── Shared top-bar leave button ───────────────────────────────────
  const leaveBtn = (compact) => (
    <button onClick={() => setShowExitModal(true)} style={{
      fontFamily:'monospace', fontSize: compact ? 10 : 12, letterSpacing:1, textTransform:'uppercase',
      background:'transparent', border:'1px solid rgba(255,51,102,0.4)',
      color:'rgba(255,51,102,0.7)', padding: compact ? '5px 10px' : '6px 14px',
      cursor:'pointer', borderRadius:4, transition:'all 0.15s', whiteSpace:'nowrap',
    }}>
      {compact ? 'Leave' : 'Leave Room'}
    </button>
  );

  // ── Mic error toast ───────────────────────────────────────────────
  const micErrorBar = micError ? (
    <div style={{ background:'rgba(255,51,102,0.12)', border:'1px solid rgba(255,51,102,0.35)', color:'#ff3366', fontFamily:'monospace', fontSize:12, padding:'7px 16px', textAlign:'center', flexShrink:0 }}>
      ⚠ {micError}
    </div>
  ) : null;
  ...
  if (isDesktop) {
    return (
      <div style={{ display:'flex', flexDirection:'column', height:'100dvh', overflow:'hidden' }}>
        {showExitModal && <ExitModal onConfirm={() => window.location.reload()} onCancel={() => setShowExitModal(false)} />}

        {/* Top bar */}
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'10px 24px', borderBottom:'1px solid rgba(245,240,232,0.1)', background:'rgba(245,240,232,0.03)', flexShrink:0, gap:12, flexWrap:'wrap' }}>
          <div style={{ display:'flex', alignItems:'center', gap:14 }}>
            <span style={{ fontFamily:'monospace', fontSize:22, color:'#c8ff00', letterSpacing:3, fontWeight:'bold' }}>UP DOWN</span>
            <span style={{ fontFamily:'monospace', fontSize:12, color:'rgba(245,240,232,0.3)', letterSpacing:2 }}>ROUND {room.round}</span>
            <span style={{ fontFamily:'monospace', fontSize:12, color:'rgba(245,240,232,0.2)', letterSpacing:2 }}>{room.code}</span>
          </div>
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', height:'100dvh', overflow:'hidden' }}>
      ...
      {/* Top bar */}
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'10px 16px', borderBottom:'1px solid rgba(245,240,232,0.1)', background:'rgba(245,240,232,0.03)', flexShrink:0 }}>
        <div style={{ display:'flex', alignItems:'center', gap:8 }}>
          <span style={{ fontFamily:'monospace', fontSize:18, color:'#c8ff00', letterSpacing:2, fontWeight:'bold' }}>UP DOWN</span>
          <span style={{ fontFamily:'monospace', fontSize:10, color:'rgba(245,240,232,0.3)', letterSpacing:1 }}>R{room.round}</span>
        </div>
        <div style={{ display:'flex', alignItems:'center', gap:8 }}>
          {room.timerEnd && <Timer timerEnd={room.timerEnd} />}
      ...
      <div style={{ display:'flex', borderTop:'1px solid rgba(245,240,232,0.1)', background:'rgba(10,10,15,0.97)', flexShrink:0, paddingBottom:'env(safe-area-inset-bottom,0px)' }}>
        {[
          ...
          <button key={tab.id} onClick={() => switchTab(tab.id)}
            style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'10px 4px', background:'none', border:'none', cursor:'pointer', color: activeTab===tab.id ? '#c8ff00' : 'rgba(245,240,232,0.35)', borderTop: activeTab===tab.id ? '2px solid #c8ff00' : '2px solid transparent', transition:'color 0.15s', position:'relative' }}>
            ...
            {tab.id==='chat' && unread>0 && (
              <span style={{ position:'absolute', top:6, right:'calc(50% - 18px)', background:'#ff3366', color:'#fff', borderRadius:'50%', width:16, height:16, fontSize:10, fontWeight:'bold', display:'flex', alignItems:'center', justifyContent:'center' }}>
                {unread > 9 ? '9+' : unread}
              </span>
            )}
          </button>
